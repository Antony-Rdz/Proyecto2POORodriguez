...
public class Email extends JFrame {
	...
	public void enviaNotasCorreo(JTable tabla,JTable tab,int fila,String nombre, String correo,String nombreExamen) {
		...
		System.out.println("HOLAASDA: "+String.valueOf(tab.getValueAt(fila, tab.getColumnCount()-1)));
		...
		System.out.println(tablaExtra.toString());
		...
		String mensaje = "<p><br></p>\r\n"
				...
				+ "                                                        <td style=\"background-color: #1b7991; color: #ffffff; text-align: center; height: 23px; width: 5%; \">Resp. Examen</td>\r\n"																		
				+ "\r\n"
				+
		...
		try {
			...
			mail = new MimeMessage(sesion);
			mail.setFrom(new InternetAddress(correoEnvia));
			//InternetAddress correoEnviado = new InternetAddress(destinatario);
			mail.addRecipient(Message.RecipientType.TO, new InternetAddress(destinatario));
			mail.setSubject(asunto);
				...
				try {
					...
					if (tablaName.equals("alumnos")) {
						...
						ps = conn.prepareStatement(
								"UPDATE profesores SET Contraseña = AES_ENCRYPT(?,'profesores') WHERE Correo = ?");
					}
